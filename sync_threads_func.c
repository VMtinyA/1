...
void sync_threads_start (void) {
    ...
    // инициализация пускового семафора потока semSI2 - поток-диспетчер приостановлен
     ...
     THREAD_ARG manager_args = {1, MAXPRIORITY_FIFO};
...
static void *thread_manager(void *param) {
    ...
    pthread_attr_t attr;
    pthread_t slave_threads[SYNCTHREADSNUM];
    THREAD_ARG slave_args[SYNCTHREADSNUM] = {
        {1, MAXPRIORITY_FIFO - 1},
        {1, MAXPRIORITY_FIFO - 2},
        {1, MAXPRIORITY_FIFO - 3}
    };
